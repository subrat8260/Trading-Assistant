  ...
  return (
    <>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/90 p-5 shadow-2xl backdrop-blur-xl space-y-4">
        ...
        {/* QUICK DATE RANGE PRESET CHIPS & CALENDAR MODAL TRIGGER */}
        <div className="pt-2 border-t border-slate-800/60 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="text-[11px] font-semibold text-slate-500 mr-1 flex items-center gap-1">
              <CalendarIcon className="h-3 w-3 text-emerald-400" /> Date Filter:
            </span>
          ...
          {/* CUSTOM POPUP CALENDAR TRIGGER BUTTON */}
          <button
            type="button"
            onClick={() => setIsDateModalOpen(true)}
            className={`flex items-center gap-2 rounded-xl border px-3.5 py-1.5 text-xs font-semibold transition-all shadow-sm active:scale-95 ${
              filters.startDate || filters.endDate
          ...
          >
            ...
            {(filters.startDate || filters.endDate) && (
              <span
                ...
                className="ml-1 rounded-full p-0.5 hover:bg-rose-500/20 hover:text-rose-400 transition-colors"
