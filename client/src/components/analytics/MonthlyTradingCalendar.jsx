...
const MonthlyTradingCalendar = ({ trades = [] }) => {
  ...
  // Active Hovered Day for Tooltip
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-950/90 p-6 shadow-2xl backdrop-blur-xl space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          {['Week', 'Month', 'Year', 'All time'].map((tab) => {
            ...
            return (
              <button
                ...
                className={`rounded-lg px-4 py-1.5 text-xs font-bold transition-all ${isActive
        ...
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-bold text-white tracking-wide min-w-[160px] text-center">
            {monthName} {year}
          </h2>
          <button
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 shadow-lg flex flex-col justify-between">
          ...
          <div
            className={`text-3xl font-black tracking-tight mt-2 ${totalMonthlyPnL >= 0 ? 'text-emerald-400' : 'text-rose-400'
              }`}
          >
            {totalMonthlyPnL >= 0 ? '+' : ''}
            ...
            {isEditingGoal ? (
              <div className="flex items-center gap-1">
                <input
                  ...
                  className="w-24 rounded-lg border border-slate-700 bg-slate-950 px-2 py-1 text-xs text-white focus:border-emerald-500 focus:outline-none"
                />
                <button
      ...
      <div className="space-y-3">
        {/* Day Names Header */}
        <div className="grid grid-cols-7 gap-2 text-center text-xs font-extrabold uppercase tracking-wider text-slate-400">
        ...
        <div className="grid grid-cols-7 gap-2">
          ...
          {Array.from({ length: firstDayOfWeek }).map((_, idx) => (
            <div
              key={`offset-${idx}`}
              className="min-h-[95px] rounded-2xl border border-dashed border-slate-900/50 bg-slate-950/20"
            />
          ))}
          ...
          {Array.from({ length: totalDaysInMonth }).map((_, idx) => {
            ...
            return (
              <div
                ...
                className={`relative min-h-[95px] rounded-2xl p-3 flex flex-col justify-between transition-all duration-200 cursor-pointer shadow-md group ${hasTrades
                    ? isProfit
                      ? 'bg-emerald-600/90 hover:bg-emerald-500 border border-emerald-400/50 text-white shadow-emerald-900/20 hover:scale-[1.03]'
                      : isLoss
                        ? 'bg-rose-600/90 hover:bg-rose-500 border border-rose-400/50 text-white shadow-rose-900/20 hover:scale-[1.03]'
                        : 'bg-slate-800 border border-slate-700 text-white hover:scale-[1.03]'
              ...
              >
                ...
                <div className="my-1 text-center">
                  {hasTrades ? (
                    <div className="text-sm font-black tracking-tight drop-shadow-sm">
                      {isProfit ? '+' : ''}
                ...
                <div className="text-center text-[10px] font-semibold opacity-90">
                  {hasTrades
                ...
                {hoveredDayData?.day === dayNum && hasTrades && (
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 w-52 rounded-2xl border border-slate-700 bg-slate-900 p-3.5 shadow-2xl backdrop-blur-xl text-xs space-y-2 pointer-events-none animate-in fade-in zoom-in-95 duration-150">
